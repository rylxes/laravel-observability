...
class StoreTraceJobTest extends TestCase
{
    public function test_it_implements_should_queue(): void
    {
        $this->assertTrue(
            in_array(ShouldQueue::class, class_implements(StoreTraceJob::class))
        );
    }

    public function test_it_uses_required_traits(): void
    {
        $traits = class_uses_recursive(StoreTraceJob::class);
    ...
    public function test_constructor_accepts_trace_data(): void
    {
        $reflection = new \ReflectionClass(StoreTraceJob::class);
        $constructor = $reflection->getConstructor();

        $this->assertNotNull($constructor);
        $this->assertCount(1, $constructor->getParameters());
        $this->assertEquals('traceData', $constructor->getParameters()[0]->getName());
    }
